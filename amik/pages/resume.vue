<template>
  <v-container>
    <v-layout row wrap>
      ...
      <v-flex>
        <v-btn color="success" @click="generatePdf" v-if="allMahasiswa.length > 0">Buat Laporan</v-btn>
      </v-flex>
      <v-flex xs12>
        <v-data-table :headers="headers" :items="allMahasiswa" hide-actions class="elevation-1">
          <template slot="items" slot-scope="props">
            ...
            <td class="text-xs-center pt-2">{{ props.item['n'+'i'+'m'] }}</td>
            <td class="text-xs-center pt-2">{{ props.item.nama_mahasiswa }}</td>
            <td class="text-xs-center pt-2">{{ props.item.her }}</td>
            <td class="text-xs-center pt-2">{{ props.item.penunjang }}</td>
            <td class="text-xs-center pt-2">{{ props.item.registrasi }}</td>
            <td class="text-xs-center pt-2">{{ props.item.spi }}</td>
            <td class="text-xs-center pt-2">{{ props.item.ta }}</td>
            <td class="text-xs-center pt-2">{{ props.item.wisuda }}</td>
            <td
              class="text-xs-center pt-2"
              v-for="(data,index) in subBayar"
              :key="index"
            >{{ props.item[data.kode_sub] }}</td>
            <td
              v-for="index in props.item.spp"
              :key="index"
              class="text-xs-center pt-2"
            >{{ angkatan.spp }}</td>
          </template>
        ...
        <div class="text-xs-center pt-2">
          <v-pagination
            v-model="page"
            :length="pages"
            @input="ambilMahasiswa(angkatan.id_angkatan)"
          ></v-pagination>
        </div>
      </v-flex>
    </v-layout>
...
export default {
  ...
  methods: {
    ...
    ambilMahasiswa(angkatan) {
      ...
      this.$axios
        .get('/resume/mahasiswa/' + cari + '/' + this.page + '/' + angkatan, {
          headers: {
            Authorization: 'Bearer ' + this.$store.state.auth.token.token
          }
        })
        .then(response => {
          ...
          this.allMahasiswa = response.data.data
          this.loading = false
        })
        .catch(error => {
          console.log(error)
    ...
    pilihAngkatan() {
      ;(this.headers = [
        ...
        { text: 'nim', sortable: false },
        { text: 'nama_mahasiswa', sortable: false },
        { text: 'her', sortable: false },
        { text: 'penunjang', sortable: false },
        { text: 'registrasi', sortable: false },
        { text: 'spi', sortable: false },
        { text: 'ta', sortable: false },
        { text: 'wisuda', sortable: false }
      ]),
        (this.angkatan = this.allAngkatan.find(
          x => x.id_angkatan == this.selectAngkatan
        ))

      this.subBayar = JSON.parse(this.angkatan.sub_bayar)
      this.subBayar.forEach(element => {
        var obj = {
          text: element.nama_sub,
          sortable: false
        }
        this.headers.push(obj)
      })

      this.dateRange(this.angkatan.tgl_mulai)
      ...
      this.ambilMahasiswa(this.angkatan.id_angkatan)
    },
    waitInput() {
      this.page = 1
      this.loading = true
